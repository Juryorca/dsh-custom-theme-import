    ...
    if (pack && pack.moduleId) {
      try {
        ...
        if (typeof applyFn === 'function') {
          // Official DSH services we can offer to every loaded skin/plugin.
          // They are added here once, not per skin. package.json
          // `dsh.client.inject` entries are module IDs for the DSH loader,
          // not Cordis service keys, so they are not used for ctx.get().
          ...
          const candidateInject = Array.from(new Set([...OFFICIAL_SERVICES, ...modInject]))
          const missing = []
          const resolvedInject = []
          for (const name of candidateInject) {
            let present = false
            try { present = typeof ctx.get(name) !== 'undefined' } catch { present = false }
            if (present) {
              resolvedInject.push(name)
            } else if (modInject.includes(name)) {
              // A skin explicitly asked for a service we cannot provide.
              missing.push(name)
            }
            // Optional official services that are absent are simply skipped.
          }
          if (missing.length > 0) {
