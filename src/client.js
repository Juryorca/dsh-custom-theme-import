...
function ThemeCard({ api }) {
  ...
  return h('div', null, [
    ...
    noticeBlock,
    h('div', { key: 'toolbar', style: { ...rowStyle, display: 'flex', alignItems: 'center', gap: '12px' } }, [
      h('button', { key: 'refresh', onClick: () => refresh(false), style: buttonStyle }, '刷新'),
      h('label', { key: 'auto', style: { display: 'flex', alignItems: 'center', gap: '6px' } }, [
        h('input', { key: 'auto-input', type: 'checkbox', checked: state.autoRefresh, onChange: (e) => setState((current) => ({ ...current, autoRefresh: e.target.checked })) }),
        '自动刷新（5 秒）',
      ]),
    ]),
